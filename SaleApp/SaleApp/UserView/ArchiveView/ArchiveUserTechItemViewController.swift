//
//  ArchiveUserTechItemViewController.swift
//  SaleApp
//
...
class ArchiveUserTechItemViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    private lazy var noItemsLabel: UILabel = {
        let label = UILabel()
        label.text = "No archived tech items available"
    ...
    private func setupTableView() {
        ...
        tableView.register(UserTechItemTableViewCell.self, forCellReuseIdentifier: "UserTechItemCell")
    ...
    private func fetchTechItems() {
        ...
        techDataManager.databaseRef.child("techItems").queryOrdered(byChild: "userId").queryEqual(toValue: userId).observe(.value) { snapshot in
            ...
            // Filter items where status is 'archiveListing'
            self.techItems = fetchedItems.filter { $0.status == .archiveListing }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        guard let cell = tableView.dequeueReusableCell(withIdentifier: "UserTechItemCell", for: indexPath) as? UserTechItemTableViewCell else {
...
class ArchiveUserTechItemTableViewCell: UITableViewCell {
